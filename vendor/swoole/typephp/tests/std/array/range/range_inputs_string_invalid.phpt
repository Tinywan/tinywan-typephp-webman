...
Warning:%S Argument #1 ($start) must be a single byte, subsequent bytes are ignored in %s on line %d

Warning:%S Argument #2 ($end) must be a single byte, subsequent bytes are ignored in %s on line %d
array(2) {
  [0]=>
  string(1) "A"
  [1]=>
...
Warning:%S Argument #2 ($end) must not be empty, casted to 0 in %s on line %d
...
Warning:%S Argument #1 ($start) must not be empty, casted to 0 in %s on line %d
...
Warning:%S Argument #3 ($step) must be of type int when generating an array of characters, inputs converted to 0 in %s on line %d
array(1) {
  [0]=>
  float(0)
}
